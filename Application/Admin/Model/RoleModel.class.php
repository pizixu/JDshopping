...
class RoleModel extends Model 
{
	...
	protected $_validate = array(
		...
		array('role_name', '1,30', '角色名称不能重复！', 1, 'unique', 3),
	);
	public function search($pageSize = 20)
	{
		...
		$data['page'] = $page->show();
		/************************************** 取数据 ******************************************/
		/*SELECT a.*,GROUP_CONCAT(c.pri_name) pri_name FROM sp_role a LEFT JOIN
sp_role_privilege b ON a.id=b.role_id LEFT JOIN sp_privilege c ON b.pri_id
=c.id GROUP BY a.id;*/
		$data['data'] = $this->field('a.*,GROUP_CONCAT(c.pri_name) pri_name')->alias('a')->join('LEFT JOIN sp_role_privilege b ON a.id=b.role_id LEFT JOIN sp_privilege c ON b.pri_id=c.id')->where($where)->group('a.id')->limit($page->firstRow.','.$page->listRows)->select();
		return $data;
	}
	// 添加前
