...
class IndexController extends Controller 
{
	public function __construct()
	{
		...
		if($adminId == 1)
			$sql = 'SELECT COUNT(*) has FROM sp_privilege WHERE '.$where;
		else 
			$sql = 'SELECT COUNT(a.role_id) has
			  FROM sp_role_privilege a
			   LEFT JOIN sp_privilege b ON a.pri_id=b.id
			   LEFT JOIN sp_admin_role c ON a.role_id=c.role_id
	...
	public function setPageBtn($page_title,$page_btn_name,$page_btn_link){
